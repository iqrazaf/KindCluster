...
PNS_CSV = Path("data/Probability_Table/cross_shop_PNS_T3.csv") #PN,PS,PNS Values
RT_CSV = Path("data/RT_data/RT_T3_S1.csv") #Response Time Values per Trial
...
# -------------------------
# Service name mapping (STIGs→ trace names)
# -------------------------
...
def map_service_name_from_stig(name: str) -> str:
    """
    Map node labels from STIGs graphs to the short service names
...
# =========================
#   Objective (full placement)
# =========================

def make_objective_full(
    target_services,
    latency_ms,
    interference,
    comm_edges,
    ...
    def evaluate(trial: optuna.Trial):
        ...
        total_latency = sum(latency_ms.get(s, 1000.0) for s in target_services)
...
def main():
    ...
    study_full = optuna.create_study(directions=["minimize", "minimize"],
                                     sampler=sampler)
    study_full.optimize(
        make_objective_full(target_services, latency_ms, interference, comm_edges),
        n_trials=200,
    )

    print_best_solution(f"Best Full Placement{title_suffix}", study_full, target_services)

    # Pareto front plot per trial
    tag = trial_label or "noTag"
    plot_pareto(study_full, filename=f"pareto_front_{tag}.png")
